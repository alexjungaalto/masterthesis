...
Everything below is organised around this one abstraction. There are no separate
``attacks'' --- only different \emph{targets} (the whole function, a restriction
of it, or a functional of it) recovered through the same evaluation operator.
...
\begin{enumerate}[label=\textbf{(P\arabic*)},leftmargin=*]
  ...
  $d \ll |\Voc|$. Hence $\mathrm{range}(f_\theta)$ lies in a $d$-dimensional
  subspace (affine after softmax centring). The effective output dimension is
  $d$, not $|\Voc|$.
  ...
  smooth map on a continuous, comparatively low-dimensional domain rather than an
  arbitrary function on a combinatorial set.
  \item \textbf{Lipschitz smoothness.} Bounded attention/MLP operator norms make
  $f_\theta$ Lipschitz in the embedding domain: nearby contexts produce nearby
  logits. This controls ``spikiness'' and is exactly the condition under which
  point evaluations are informative (Section~\ref{sec:rip}).
  ...
  the same domain --- polynomial in $1/\varepsilon$ with an exponent set by the
  network's effective complexity, not by the ambient domain dimension.
\end{enumerate}
...
$\Fclass$ together with Fano's inequality lower-bounds the query complexity by the
metric entropy: no recovery scheme --- adaptive or not --- can identify $f_\theta$
to accuracy $\varepsilon$ from fewer than $\sim\Hent_\varepsilon(\Fclass)$
informative evaluations. This pins the query complexity to the \emph{intrinsic}
complexity of the model class, independent of vocabulary size or context length
except through $\Hent_\varepsilon$.
